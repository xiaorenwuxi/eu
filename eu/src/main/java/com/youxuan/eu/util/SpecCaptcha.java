	    ...
	    private boolean graphicsImage(char[] strs, OutputStream out){
	        ...
	        try
	        {
	            ...
	            for(int i=0;i<15;i++){
	                color = color(150, 250);
	                g.setColor(color);
	                g.drawOval(ran.num(width), ran.num(height), 5+ran.num(10), 5+ran.num(10));// 画蛋蛋，有蛋的生活才精彩
	                color = null;
	            }
	            g.setFont(font);
	            ...
	            for(int i=0;i<len;i++)
	            {
	                ...
	                g.setComposite(ac3);
	                color = new Color(20 + ran.num(110), 20 +ran. num(110), 20 + ran.num(110));// 对每个字符都用随机颜色
	                g.setColor(color);
