	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="'+"http://localhost:8080/eu"+para.url+".do"+'" method="post" enctype="multipart/form-data">';
					html += '	<div class="upload_box">';
					html += '		<div class="upload_main">';
					html += '			<div class="upload_choose">';
	            	html += '				<div class="convent_choice">';
	            	html += '					<div class="andArea">';
	            	html +='                          <div id="myaddcss">   ';
	            	html += '						<div class="filePicker">点击上传</div>';
	            	html += '	                       <div class="upload_btn">开始上传</div>';
	            	html +='                               </div>'
	            	html += '						<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
					html += '				<span id="fileDragArea" class="upload_drag_area"></span>';
					html += '			</div>';
					  ...
					  if(successpath.length>0)
						...
						for(var i=0;i<successpath.length;i++)
							...
							if(successpath[i].indexOf(file.name) > 0)
								{
								$.post("upload/deleteFile.do",{file:successpath[i]});
